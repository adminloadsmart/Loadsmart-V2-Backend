...
export class AuthRepository {
    ...
    findActiveRefreshTokenByHash(tokenHash: string): Promise<RefreshTokenEntity | null> {
        return this.refreshTokens.findOneBy({ tokenHash, revokedAt: IsNull(), expiresAt: MoreThan(new Date()) });
    }
    ...
    async recordLoginAttempt(data: { email: string; success: boolean; ipAddress: string | null }): Promise<void> {
    ...
    countRecentFailedAttempts(email: string, ipAddress: string | null, sinceMs: number): Promise<number> {
