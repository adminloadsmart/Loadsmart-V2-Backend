...
export class RoleRepository {
    ...
    findUserWithRole(userId: string): Promise<UserEntity | null> {
        return this.users.findOne({ where: { id: userId }, relations: { role: { permissions: true } } });
    }
    ...
    findUserPermissionGrant(userId: string, permissionId: string): Promise<UserPermissionEntity | null> {
        return this.userPermissions.findOneBy({ userId, permissionId });
    }

    async grantPermission(userId: string, permissionId: string, grantedBy: string): Promise<UserPermissionEntity> {
