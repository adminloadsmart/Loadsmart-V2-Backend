...
export class RoleService {
    ...
    async listRoles(actingUser: AuthenticatedUser, filters: { scope?: RoleScope }): Promise<RoleEntity[]> {
    ...
    async listPermissions(actingUser: AuthenticatedUser, filters: { scope?: PermissionScope }): Promise<PermissionEntity[]> {
        this.assertCanViewCatalog(actingUser);
        try {
    ...
    async getEffectivePermissions(userId: string): Promise<string[]> {
        ...
        return [...new Set([...user.role.permissions.map((p) => p.key), ...grants.map((g) => g.permission.key)])];
    }

    async getUserPermissions(actingUser: AuthenticatedUser, targetUserId: string): Promise<EffectiveUserPermissions> {
        try {
    ...
    assertCanManage(actingUser: AuthenticatedUser, targetUser: UserEntity, thingScope: Scope): void {
        ...
        if (actingUser.role === PLATFORM_ADMIN_ROLE) {
            if (thingScope !== 'platform') {
                throw new AuthorizationError('Platform admin can only manage platform-scope roles/permissions');
            }
            return;
        }
        if (actingUser.role === ORG_ADMIN_ROLE) {
            if (thingScope !== 'organization') {
                throw new AuthorizationError('Org admin can only manage organization-scope roles/permissions');
            }
    ...
    async assignRole(actingUser: AuthenticatedUser, targetUserId: string, roleId: string): Promise<RoleEntity> {
        try {
    ...
    async grantPermission(actingUser: AuthenticatedUser, targetUserId: string, permissionId: string): Promise<PermissionEntity> {
        try {
            const targetUser = await this.roleRepository.findUserWithRole(targetUserId);
            if (!targetUser) throw new NotFoundError(`User ${targetUserId} not found`);
            ...
            const existing = await this.roleRepository.findUserPermissionGrant(targetUserId, permissionId);
            if (existing) throw new ConflictError(`User already has permission "${permission.key}" granted directly`);
    ...
    async revokePermission(actingUser: AuthenticatedUser, targetUserId: string, permissionId: string): Promise<void> {
        try {
            const targetUser = await this.roleRepository.findUserWithRole(targetUserId);
            if (!targetUser) throw new NotFoundError(`User ${targetUserId} not found`);
            const permission = await this.roleRepository.findPermissionById(permissionId);
            ...
            const existing = await this.roleRepository.findUserPermissionGrant(targetUserId, permissionId);
            if (!existing) throw new NotFoundError(`User does not have permission "${permission.key}" granted directly`);
