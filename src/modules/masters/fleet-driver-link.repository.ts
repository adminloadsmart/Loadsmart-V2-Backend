...
import { CreateFleetDriverLinkData, UpdateFleetDriverLinkData } from './utils/fleet-driver-link.interface';
...
export class FleetDriverLinkRepository {
  ...
  async create(data: CreateFleetDriverLinkData, manager?: EntityManager): Promise<FleetDriverLinkEntity> {
  ...
  findActiveLink(tenantId: string, vehicleId: string, driverId: string): Promise<FleetDriverLinkEntity | null> {
    return this.links.findOneBy({ tenantId, vehicleId, driverId, status: 'active', deletedAt: IsNull() });
  }
  ...
  async update(tenantId: string, id: string, data: UpdateFleetDriverLinkData, manager?: EntityManager): Promise<FleetDriverLinkEntity | null> {
    const links = manager ? manager.getRepository(FleetDriverLinkEntity) : this.links;
  ...
  async softDelete(tenantId: string, id: string, deletedBy: string | null): Promise<void> {
    await this.links.update({ id, tenantId, deletedAt: IsNull() }, { deletedAt: new Date(), updatedBy: deletedBy });
  }
}
